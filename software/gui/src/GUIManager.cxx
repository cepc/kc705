...
void GUIManager::config(){
  ...
  //TODO: get the path of json, from GUI
  std::string json_path = "sample_summer.json";
  std::string str_json = JadeUtils::LoadFileToString(json_path);
  JadeOption opt_conf(str_json);
  JadeOption opt_(str_json);
  
  m_man = JadeManager::Make("TestManager",  JadeOption(str_json));
  m_man->Init();
