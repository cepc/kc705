...
void JadeDataFrame::Decode(){
  ...
  size_t p_offset = 0;
  uint64_t header64 = LE64TOH(*reinterpret_cast<const uint64_t*>
			      (p_raw+p_offset));
  if(header64 != 0xaaaaaaaa){
  ...
  uint64_t footer64 = LE64TOH(*reinterpret_cast<const uint64_t*>
			      (p_raw+p_offset));
  if(footer64 != 0xaaaaaaaa){
